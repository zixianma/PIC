...
# eval_agent.save_model(args.exp_name, exp_save_dir)
# torch.save(eval_agent.state_dict(), os.path.join(exp_save_dir, 'agents_best.ckpt'))
# torch.save(eval_agent, os.path.join(exp_save_dir, 'agents_best.ckpt'))
 # log
...
for i_episode in range(args.num_episodes):
        ...
        if len(memory) > args.batch_size:
            if total_numsteps % args.steps_per_actor_update == 0:
                for _ in range(args.updates_per_step):
                    ...
                    # bacth = process_fn(batch, num_agents=n_agents)
                    transitions, _ = memory.sample(args.batch_size)
                    batch = Transition(*zip(*transitions))
                    policy_loss = agent.update_actor_parameters(batch, i, args.shuffle)
                    inverse_loss, forward_loss = agent.update_inverse_and_forward_parameters(batch, i, args.shuffle)
                    updates += 1
                writer.add_scalar('policy_loss/train', policy_loss, i_episode)
                writer.add_scalar('inverse_loss/train', inverse_loss, i_episode)
                writer.add_scalar('forward_loss/train', forward_loss, i_episode)
    ...
    if (i_episode + 1) % args.eval_freq == 0:
        tr_log = {'num_adversary': 0,
                  ...
                  'inverse_loss': inverse_loss,'forward_loss': forward_loss,
